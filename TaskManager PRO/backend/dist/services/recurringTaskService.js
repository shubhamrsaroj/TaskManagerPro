...
exports.generateUpcomingRecurringTasks = exports.processCompletedRecurringTasks = exports.createRecurringTaskInstance = void 0;
const Task_1 = require("../models/Task");
const index_1 = require("../index");
/**
 * Creates a new instance of a recurring task
 */
const createRecurringTaskInstance = async (parentTask) => {
    try {
        ...
        const newTaskInstance = new Task_1.Task({
        ...
        if (newTaskInstance.assignedTo.toString() !== newTaskInstance.createdBy.toString()) {
            await (0, index_1.createNotification)(newTaskInstance.assignedTo.toString(), `New recurring task: ${newTaskInstance.title}`, 'task_assigned', newTaskInstance._id.toString());
        }
...
exports.createRecurringTaskInstance = createRecurringTaskInstance;
/**
...
const processCompletedRecurringTasks = async () => {
    try {
        ...
        const completedRecurringTasks = await Task_1.Task.find({
            isRecurring: true,
            status: 'completed',
        });
        for (const task of completedRecurringTasks) {
            await (0, exports.createRecurringTaskInstance)(task);
        }
...
exports.processCompletedRecurringTasks = processCompletedRecurringTasks;
/**
 * Find all tasks that need to be created based on their recurring pattern
 * This should be run on a schedule (e.g., daily)
 */
const generateUpcomingRecurringTasks = async () => {
    try {
        // Get all recurring tasks without a parent (main recurring tasks)
        const recurringTasks = await Task_1.Task.find({
            isRecurring: true,
        ...
        for (const task of recurringTasks) {
            ...
            const existingInstances = await Task_1.Task.countDocuments({
            ...
            if (existingInstances === 0) {
                const newInstance = await (0, exports.createRecurringTaskInstance)(task);
                if (newInstance)
                    createdCount++;
            }
        }
...
exports.generateUpcomingRecurringTasks = generateUpcomingRecurringTasks;
